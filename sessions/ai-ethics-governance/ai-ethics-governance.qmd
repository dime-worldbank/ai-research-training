...
<div class="poll-grid">
  <div class="poll-row" data-poll="w1" data-idx="0"><button class="poll-btn" onclick="pollVote('w1',0)">Uploaded an Official Use Only document without checking whether the AI account was approved for it?</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="1"><button class="poll-btn" onclick="pollVote('w1',1)">Used an AI-generated table or figure without checking its source or underlying code?</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="2"><button class="poll-btn" onclick="pollVote('w1',2)">Accepted a result because it looked right without checking the sources</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="3"><button class="poll-btn" onclick="pollVote('w1',3)">Given a coding assistant access to a folder containing non public data?</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="4"><button class="poll-btn" onclick="pollVote('w1',4)">Used AI-generated text in a paper or presentation without explaining its contribution?</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
</div>
```
...
::: {.lede-line}
With AI there are a lot of judgment calls. These are not.
:::

 - Cause or exacerbate harm, whether individual or collective, to social, cultural, economic, natural, and political environments.
- Promotes bias, discrimination and stigmatization of individuals or groups.
- Prevents humans from overriding AI generated decisions.
- Actively hides the use of AI.
- Are purposefully manipulative or uses deceptive techniques.
- Infer emotions of a human (e.g. client or counterpart).
...
We will present different scenario for the differnt risks, A, B, C. 

- **A situation**, drawn from real work.
...
::: {.lede-line}
Mexico's poverty rate is not a contested number. It has an official measurement, published by **INEGI**. In September 2026 I asked three assistants for it, in **English** and in **Spanish**.
:::
...
::: {.consensus-band}
Every answer lands near the official figures &mdash (in this case). The numbers agree, so nothing looks wrong. Underneath: **not one linked CONEVAL**, **two never linked INEGI at all**, **only Gemini Enterprise** linked the World Bank and **what the model read changed with the language I asked in**.
:::
