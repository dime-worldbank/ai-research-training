...
<div class="poll-grid">
  <div class="poll-row" data-poll="w1" data-idx="0"><button class="poll-btn" onclick="pollVote('w1',0)">Shared an official-use-only file with ChatGPT or Claude</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="1"><button class="poll-btn" onclick="pollVote('w1',1)">Used AI-generated charts or tables without checking the underlying code</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  ...
  <div class="poll-row" data-poll="w1" data-idx="3"><button class="poll-btn" onclick="pollVote('w1',3)">Uploaded personally identifiable data to Claude for tasks like cleaning</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
  <div class="poll-row" data-poll="w1" data-idx="4"><button class="poll-btn" onclick="pollVote('w1',4)">Used an AI-generated indicator value without checking the source</button><div class="poll-bar-track"><div class="poll-bar-fill"></div></div><span class="poll-count">0</span></div>
...
::: {.quote-big}
Whatever you clicked, you weren't the only one. That's not a coincidence &mdash; the rules for AI are new, unevenly known, and nobody's really taught them yet. This hour is that teaching.
:::
...
::: {.case-line}
**Real case · United States, 2019:** the same mistake, made and measured elsewhere. A health algorithm applied to about 200 million people a year needed a proxy for health need, so it was trained to predict healthcare *cost* instead. Because less is historically spent treating Black patients at the same level of illness, cost quietly measured *access* to care, not *need* for it &mdash; equally sick Black patients scored as healthier. Fixing it would have raised the share of Black patients flagged for extra care from 17.7% to 46.5%. Different sector, same untested proxy. ([Obermeyer et al. 2019](https://doi.org/10.1126/science.aax2342))
:::

::: {.notes}
Say: the model is not wrong, it is doing exactly what it was trained to do. The bias is in the choice of proxy, made before anyone ran the model.

The real case is the same mechanism in a different sector, with numbers attached, so it stops being hypothetical.
:::
...
3. Keep a record of what you asked AI and how you checked it.
4. Disclose meaningful AI assistance instead of staying silent. ([RRR: Documenting AI use](https://worldbank.github.io/wb-reproducible-research-repository/guidance/AI_reproducibility_guidelines.html))
5. If you're not sure, ask ai@worldbankgroup.org.
